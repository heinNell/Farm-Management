...
import { Bar, BarChart, CartesianGrid, Line, LineChart, ReferenceLine, ResponsiveContainer, Scatter, ScatterChart, Tooltip, TooltipProps, XAxis, YAxis } from 'recharts'
...
interface ProcessedEfficiencyData {
  id: string
  asset_name: string
  asset_type: string
  date: string
...
export const FuelEfficiencyChart: React.FC<FuelEfficiencyChartProps> = ({ 
  ...
  // Get benchmark status based on fuel consumption
  const getBenchmarkStatus = useCallback((assetType: string, fuelPerHour: number): BenchmarkStatus => {
  ...
  const processedData = useMemo((): ProcessedEfficiencyData[] => {
    ...
    return data
      ...
      .map(record => {
        ...
        const processedRecord: ProcessedEfficiencyData = {
          id: record.id,
  ...
  }, [data, selectedAssetType, timeRange, getBenchmarkStatus])
  ...
  const trendData = useMemo((): TrendDataItem[] => {
    ...
    const dailyData = processedData.reduce((acc: Record<string, DailyAggregation>, record) => {
      ...
      if (dayData) {
        dayData.totalFuelPerHour += record.fuel_per_hour
        dayData.totalEfficiencyRating += record.efficiency_score
        dayData.count += 1
      }
  ...
  const comparisonData = useMemo((): ComparisonDataItem[] => {
    ...
    const assetData = processedData.reduce((acc: Record<string, AssetAggregation>, record) => {
      ...
      const assetInfo = acc[key]
      if (assetInfo) {
        assetInfo.totalFuelPerHour += record.fuel_per_hour
        assetInfo.totalEfficiencyRating += record.efficiency_score
        assetInfo.count += 1
      }
    ...
    return Object.values(assetData)
      .map(asset => {
        const avgFuelPerHour = asset.totalFuelPerHour / asset.count
        return {
          ...
          benchmark_status: getBenchmarkStatus(asset.asset_type, avgFuelPerHour)
        }
      })
      .sort((a, b) => a.avgFuelPerHour - b.avgFuelPerHour)
  }, [processedData, analysisType, getBenchmarkStatus])
  ...
  const renderChart = (): React.ReactElement => {
    switch (analysisType) {
      ...
      default:
        return <div className="text-center text-gray-500 py-8">Select an analysis type</div>
    }
  }
